...
<div class="table-responsive">
    ...
    <?php if (!empty($partidasAdiantadas)) : ?>
        <h3>Jogos adiantados</h3>
        <table class="table table-striped table-hovered">
            <thead>
                <tr>
        	...
        	<tbody>
        		<?php foreach ($partidasAdiantadas as $l => $adiantado) : ?>
		            <?php $disabled = ((strtotime(date('Y-m-d')) <= strtotime($adiantado->data->format('Y-m-d'))) && (strtotime(date('H:i')) < strtotime($prazoHora[$adiantado->id]))) ? '' : 'disabled'; ?>
                	<tr>
                        <td class="d-none d-md-table-cell"><?= h($adiantado->data->format('d/m/Y')) ?></td>
                        <td class="d-none d-md-table-cell"><?= h($adiantado->horario->format('H:i')) ?></td>
                        <td class="d-none d-md-table-cell"><?= $adiantado->rodada->numero_rodada ?></td>
                        <td>
                            <div class="row">
                                <div class="col-8 col-sm-6 col-md-4 col-lg-3 col-xl-3">
                                    <?= $this->Html->image($adiantado->mandante->brasao, ['class' => 'img-fluid', 'alt' => $adiantado->mandante->descricao, 'title' => $adiantado->mandante->descricao]) ?>
                                </div>
                                <div class="col-12 col-sm-6 col-md-6 col-lg-3 col-xl-3">
                                    <input type="hidden" id="jogo-id-adiantado-<?= $l; ?>" value="<?= $adiantado->id; ?>" />
                                    <input type="hidden" id="id-aposta-adiantado-<?= $l; ?>" value="<?= $apostasAdiantadas[$adiantado->id]['id']; ?>" />
                                    <input type="hidden" id="time1-adiantado-<?= $l; ?>" value="<?= $adiantado->casa; ?>" />
                                    <input type="number" <?= $disabled ?> onBlur="salvarLance(this.value, null, <?= $l ?>, <?= $adiantado->id ?>);" min="0" id="placar-mandante-adiantado-<?= $l ?>" class="form-control" value="<?= $apostasAdiantadas[$adiantado->id]['time1'] ?>" />
                                </div>
                                <div class="col">
                                    <?= $adiantado->placar1; ?>
                                </div>
                        ...
                        <td>
                            <div class="row">
                                <div class="col-8 col-sm-6 col-md-4 col-lg-3 col-xl-3">
                                    <?= $this->Html->image($adiantado->fora->brasao, ['class' => 'img-fluid', 'alt' => $adiantado->fora->descricao, 'title' => $adiantado->fora->descricao]) ?>
                                </div>
                                <div class="col-12 col-sm-6 col-md-6 col-lg-3 col-xl-3">
                                    <input type="hidden" id="time2-adiantado-<?= $l; ?>" value="<?= $adiantado->visitante; ?>" />
                                    <input type="number" <?= $disabled ?> onBlur="salvarLance(null, this.value, <?= $l ?>, <?= $adiantado->id ?>);" min="0" id="placar-visitante-adiantado-<?= $l ?>" class="form-control" value="<?= $apostasAdiantadas[$adiantado->id]['time2'] ?>" />
                                </div>
                                <div class="col">
                                    <?= $adiantado->placar2; ?>
                                </div>
                        ...
                        <?php if (in_array($usuario['group_id'], [1])) : ?>
                            <td><?php echo $adiantado->id; ?></td>
                            <td class="actions">
                                <?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $adiantado->id], ['escape' => false, 'title' => 'Editar registro']) ?>
                                <?= $this->Form->postLink('<i class="fas fa-trash-alt text-danger"></i>', ['action' => 'delete', $adiantado->id], ['confirm' => __('Deseja excluir o registro # {0}?', $adiantado->id), 'escape' => false, 'title' => 'Excluir registro']) ?>
                            </td>
                        <?php endif; ?>
                        <!-- <td>
                        <?php echo $this->Html->link('Apostas', ['controller' => 'Apostas', 'action' => 'index', $adiantado->id], ['title' => 'Apostas']); ?>
                        </td> -->
                    </tr>
                <?php endforeach; ?>
        	</tbody>
...
if (!empty($partidasAdiantadas)) {
    $l = (isset($l)) ? $l : 0;
    echo $this->Html->scriptBlock('let adiantados = true; quantidadeApostas = parseInt(' . ($key + $l) . ');', ['block' => 'scriptEnd']);
}
